...
bestAngles = load("/scratch/qz886/bestAngles_15res.mat").bestAngles;
bestScores = load("/scratch/qz886/bestAngles_15res.mat").bestScores;

N = length(bestScores);
TorsionBins = zeros(N,15);
...
candPhi = zeros(15, length(ClusterScores));
candPsi = zeros(15, length(ClusterScores));
candCoordinates_x = zeros(15, length(ClusterScores));
candCoordinates_y = zeros(15, length(ClusterScores));
candCoordinates_z = zeros(15, length(ClusterScores));
...
for i = 1 : size(TorsionBins,1)
    bin = TorsionBins(i,:);
    angles = reshape(bestAngles(:,i), [2,15]);
    ...
    if length(indices) ~= length(bin)
        ...
        PermAngles(:,i) = reshape(circshift(angles, length(bin)-indices+count+1, 2), [30, 1]);
    end
end
end
