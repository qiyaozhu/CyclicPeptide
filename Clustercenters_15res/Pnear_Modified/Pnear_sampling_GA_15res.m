function Pnear_sampling_GA_15res(seq, name)
...
filename = sprintf('After_GA_%s_15res.mat', name);
data = load(filename);
Angles = data.Angles;
Coordinates = data.Coordinates;
RMSD = data.RMSD;
...
while i <= M && stall < stall_threshold
    ...
    FILE = fopen("relax_"+name+"_Generation"+i+"_15res.flags", 'w');
    fprintf(FILE, "-in:file:l "+name+"_Generation"+i+"_list_15res.txt\n");
    ...
    fprintf(FILE, "-out:file:scorefile score_"+name+"_Generation"+i+"_15res.sc\n");
    fclose(FILE);

    FILE = fopen(name+"_Generation"+i+"_list_15res.txt", 'w');
    for cand = 1 : N
        fprintf(FILE, name+"_Generation"+i+"_cand"+cand+"_15res.pdb\n");
    end
    fclose(FILE);

    FILE = fopen("run_Relax_"+name+"_Generation"+i+"_15res.sbatch", 'w');
    ...
    fprintf(FILE, "srun --mpi=pmi2 /share/apps/rosetta/2020.46.61480/openmpi/intel/2020.46.61480/main/source/bin/rosetta_scripts.mpi.linuxiccrelease @relax_"+name+"_Generation"+i+"_15res.flags\n");
    fclose(FILE);

    [~, job_id] = system("sbatch run_Relax_"+name+"_Generation"+i+"_15res.sbatch");
    ...
    tic;
    FILE = fopen("score_"+name+"_Generation"+i+"_15res.sc", 'r');
    ...
    system("rm "+name+"_Generation"+i+"*_15res*.pdb");
    system("rm "+name+"_Generation"+i+"_list_15res.txt");
    system("rm relax_"+name+"_Generation"+i+"_15res.flags");
    system("rm run_Relax_"+name+"_Generation"+i+"_15res.sbatch");
    system("rm score_"+name+"_Generation"+i+"_15res.sc");
...
for cand = 1 : N
    ...
    filename = sprintf(name+"_Generation%d_cand%d_15res.pdb", generation, cand);
