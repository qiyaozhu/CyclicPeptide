...
with open('run_Pnear_sampling_SA_15res.sbatch', 'w') as f:
    f.write("#!/bin/bash\n"+
    ...
    for l in range(len(Names)):
        name = Names[l]
        seq = Seqs[l]
        f.write('Pnear_sampling_SA_15res("'+seq+'", "clustercenter'+name+'_0001_0001")\n')           
...
with open('run_Relax_SA_15res.sbatch', 'w') as f:
    f.write('#!/bin/bash\n'+
            '#SBATCH --nodes=4\n'+
            '#SBATCH --tasks-per-node=12\n'+
            '#SBATCH --mem=60GB\n'+
    ...
    for l in range(len(Names)):
        name = Names[l]
        seq = Seqs[l]
        f.write('srun --mpi=pmi2 /share/apps/rosetta/2020.46.61480/openmpi/intel/2020.46.61480/main/source/bin/rosetta_scripts.mpi.linuxiccrelease @clustercenter'+name+'_0001_0001_SA_relax_rosetta_15res.flags\n')


with open('run_Pnear_Clustering_15res.sbatch', 'w') as f:
    f.write("#!/bin/bash\n"+
...
with open('run_Pnear_sampling_GA_15res.sbatch', 'w') as f:
    f.write("#!/bin/bash\n"+
    ...
    for l in range(len(Names)):
        name = Names[l]
        seq = Seqs[l]
        f.write('Pnear_sampling_GA_15res("'+seq+'", "clustercenter'+name+'_0001_0001")\n')
    f.write("delete(gcp('nocreate'))\n"+
            "exit\n"+
            "EOF")
