function Pnear_sampling_GA_3N(seq, name)
...
filename = sprintf('After_SA_%s_Angles.txt', name);
Peptides = readmatrix(filename).';
Scores = Peptides(3*n+1,:);
RMSD = Peptides(end,:);
Angles = Peptides(1:3*n,:);
Angles = deg2rad(Angles);
Angles = Angles(:,Scores<score_threshold);
RMSD = RMSD(Scores<score_threshold);
...
while i <= M && stall < stall_threshold
    ...
    [Coordinates_new, Labels] = GA(Angles, Coordinates, RamaMap, xover_region, mut_region, xover_size);
    ...
    FILE = fopen("relax_"+name+"_Generation"+i+"_3N.flags", 'w');
    fprintf(FILE, "-in:file:l "+name+"_Generation"+i+"_list_3N.txt\n");
    fprintf(FILE, '-in:file:fullatom\n');
    fprintf(FILE, "-in:file:native /scratch/qz886/Clustercenters_15res/Pnear_Candidates/output_"+name+".pdb\n");
    fprintf(FILE, '-parser:protocol relax_Cartesian.xml\n');
    fprintf(FILE, '-parser:script_vars Nres=15\n');
    fprintf(FILE, "-out:file:scorefile score_"+name+"_Generation"+i+"_3N.sc\n");
    fclose(FILE);

    FILE = fopen(name+"_Generation"+i+"_list_3N.txt", 'w');
    for cand = 1 : N
        fprintf(FILE, name+"_Generation"+i+"_cand"+cand+"_3N.pdb\n");
    end
    fclose(FILE);

    FILE = fopen("run_Relax_"+name+"_Generation"+i+"_3N.sbatch", 'w');
    fprintf(FILE, "#!/bin/bash\n");
    fprintf(FILE, "#SBATCH --nodes=6\n");
    fprintf(FILE, "#SBATCH --tasks-per-node=8\n");
    ...
    fprintf(FILE, "srun --mpi=pmi2 /share/apps/rosetta/2020.46.61480/openmpi/intel/2020.46.61480/main/source/bin/rosetta_scripts.mpi.linuxiccrelease @relax_"+name+"_Generation"+i+"_3N.flags\n");
    fclose(FILE);

    [~, job_id] = system("sbatch run_Relax_"+name+"_Generation"+i+"_3N.sbatch");
    ...
    while true
        ...
        if length(job_status) > 10
            job_status = job_status(15);
            disp("current status "+job_status);
        else
            break
        end
        pause(20);
    ...
    FILE = fopen("score_"+name+"_Generation"+i+"_3N.sc", 'r');
    ...
    % Of these centers, select the top ones with lowest RMSD
    [RMSD, bestIndices] = mink(RMSD, 500);
    Angles = Angles(:, bestIndices);
    Scores = Scores(bestIndices);
    Labels = Labels(bestIndices);
    Coordinates = Coordinates(:, bestIndices);
    ...
    system("rm "+name+"_Generation"+i+"*_3N*.pdb");
    system("rm "+name+"_Generation"+i+"_list_3N.txt");
    system("rm relax_"+name+"_Generation"+i+"_3N.flags");
    system("rm run_Relax_"+name+"_Generation"+i+"_3N.sbatch");
    system("rm score_"+name+"_Generation"+i+"_3N.sc");
    system("rm slurm-"+job_id+".out");

    filename = sprintf('After_GA_%s_3N.mat', name);
    save(filename, 'candAngles', 'candScores', 'candRMSD');

    if i==ceil(M/4) || i==2*ceil(M/4) || i==3*ceil(M/4) || i==M
        save("After_GA_i"+i+"_"+name+"_3N.mat", 'Angles', 'Coordinates', 'RMSD', 'Scores', 'candAngles', 'candScores', 'candRMSD');
    end
...
function [Coordinates_new, Labels] = GA(Angles, Coordinates, RamaMap, xover_region, mut_region, xover_size)

n = 15;
N = size(Angles,2); % population size
max_mut = 10; % maximum number of tries for a mutation attempt
rmsd_threshold = 0.5;
Coordinates_new = [];
...
for pair = 1 : size(combinations,1)
    ...
    if success
        Coordinates_new = [Coordinates_new, coordinates_x1, coordinates_x2];
        Labels = [Labels, 1, 1];
    end
...
% Mutation
N_mutant = N;
mutants = randperm(N, N_mutant);

for mut = 1 : N_mutant
    mut_length = randi(mut_region);
    mutRES = randperm(n, mut_length);
    angle = Angles(:, mutants(mut));

    for k = 1 : mut_length
        mutres = mutRES(k);
        perb_try = 0;
        while perb_try <= max_mut
            perb = 0.349*rand(2,1)-0.1745;
            temp_angle = angle(mutres*3-2:mutres*3-1) + perb;
            temp_angle(temp_angle<-pi) = temp_angle(temp_angle<-pi)+2*pi;
            temp_angle(temp_angle>pi) = temp_angle(temp_angle>pi)-2*pi;
            locations = floor(rad2deg(temp_angle)/10) + 19;
            if RamaMap(locations(1), locations(2), mutres) == 1
                angle(mutres*3-2:mutres*3-1) = temp_angle;
                break;
            end
            perb_try = perb_try + 1;
        end
    end
    Coordinates_new = [Coordinates_new, get_coordinates(angle)];
    Labels = [Labels, 2];
end
end
...
for cand = 1 : N
    ...
    filename = sprintf(name+"_Generation%d_cand%d_3N.pdb", generation, cand);
