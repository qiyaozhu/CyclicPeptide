...
with open('clustercenter'+name+'_0001_0001_SA_relax_rosetta_24res.flags', 'w') as f:
    f.write('-in:file:silent After_SA_clustercenter'+name+'_0001_0001_24res.txt\n'+
            '-in:file:fullatom\n'+
            ...
            '-out:file:scorefile After_SA_clustercenter'+name+'_0001_0001_24res.sc\n'+
            '-out:file:silent After_SA_clustercenter'+name+'_0001_0001_24res.silent')

with open('run_Relax_SA_'+name+'_24res.sbatch', 'w') as f:
    f.write('#!/bin/bash\n'+
            '#SBATCH --nodes=5\n'+
            '#SBATCH --tasks-per-node=12\n'+
            '#SBATCH --mem=35GB\n'+
            ...
            'srun --mpi=pmi2 /share/apps/rosetta/2020.46.61480/openmpi/intel/2020.46.61480/main/source/bin/rosetta_scripts.mpi.linuxiccrelease @clustercenter'+name+'_0001_0001_SA_relax_rosetta_24res.flags')

# Perform FastRelax
job_id = subprocess.run(['sbatch', 'run_Relax_SA_'+name+'_24res.sbatch'], capture_output=True, text=True).stdout.strip()
job_id = job_id.split()[-1]
while True:
    job_status = subprocess.run(['squeue', '-j', job_id], capture_output=True, text=True).stdout.strip()
    job_status = job_status.split('\n')
...
Angles = []
with open("After_SA_clustercenter"+name+"_0001_0001_24res.silent", 'r') as f:
    f.readline()
    f.readline()
    lines = f.readlines()
...
np.savetxt('After_SA_clustercenter'+name+'_0001_0001_Angles_24res.txt', Angles, delimiter='\t', fmt='%f')

# Run Genetic Algorithm
with open('run_Pnear_sampling_GA_'+name+'_24res.sbatch', 'w') as f:
    f.write("#!/bin/bash\n"+
            "#SBATCH --job-name=PnearGA24res_"+name+"\n"+
            "#SBATCH --nodes=5\n"+
            "#SBATCH --cpus-per-task=1\n"+
            "#SBATCH --tasks-per-node=12\n"+
            "#SBATCH --mem=35GB\n"+
