function Pnear_sampling_GA_24res(seq, name)
...
% Read from Energy SA results
filename = sprintf('After_Energy_SA_%s_Angles_24res.txt', name);
Peptides = readmatrix(filename).';
Energy_Scores = Peptides(3*n+1,:);
Energy_RMSD = Peptides(end,:);
Energy_Angles = Peptides(1:3*n,:);
Energy_Angles = deg2rad(Energy_Angles);

N = size(Energy_Angles, 2);
Energy_Coordinates = zeros(4*n, 3*N);
for cand = 1 : N
    angle = Energy_Angles(:,cand);
    Energy_Coordinates(:,3*(cand-1)+1:3*cand) = get_coordinates(angle);
end
Energy_Coordinates = reshape(Energy_Coordinates, [4*n*3, N]);
disp("A total population of "+N+" after SA.");

% Clustering to select lowest energy centers
tic;
[Energy_Scores, bestIndices] = mink(Energy_Scores, 5000);
Energy_Angles = Energy_Angles(:, bestIndices);
Energy_RMSD = Energy_RMSD(bestIndices);
Energy_Coordinates = Energy_Coordinates(:, bestIndices);

indices = clustering(Energy_Coordinates);
Energy_Angles = Energy_Angles(:, indices);
Energy_Scores = Energy_Scores(indices);
Energy_RMSD = Energy_RMSD(indices);
Energy_Coordinates = Energy_Coordinates(:, indices);
seconds = seconds + toc;
disp("A total of "+size(Energy_Angles,2)+" cluster centers.");

Energy_candAngles = Energy_Angles(:,Energy_Scores<score_threshold);
Energy_candScores = Energy_Scores(Energy_Scores<score_threshold);
Energy_candRMSD = Energy_RMSD(Energy_Scores<score_threshold);
Energy_candCoordinates = Energy_Coordinates(:,Energy_Scores<score_threshold);

filename = sprintf('After_Energy_SA_clustering_%s_24res.mat', name);
save(filename, 'Energy_Angles', 'Energy_Coordinates', 'Energy_RMSD', 'Energy_Scores', 'Energy_candAngles', 'Energy_candScores', 'Energy_candRMSD', 'Energy_candCoordinates');

% Read from RMSD SA results
filename = sprintf('After_RMSD_SA_%s_Angles_24res.txt', name);
Peptides = readmatrix(filename).';
RMSD_Scores = Peptides(3*n+1,:);
RMSD_RMSD = Peptides(end,:);
RMSD_Angles = Peptides(1:3*n,:);
RMSD_Angles = deg2rad(RMSD_Angles);

N = size(RMSD_Angles, 2);
RMSD_Coordinates = zeros(4*n, 3*N);
for cand = 1 : N
    angle = RMSD_Angles(:,cand);
    RMSD_Coordinates(:,3*(cand-1)+1:3*cand) = get_coordinates(angle);
end
RMSD_Coordinates = reshape(RMSD_Coordinates, [4*n*3, N]);
disp("A total population of "+N+" after SA.");

% Clustering to select lowest energy centers
tic;
[RMSD_Scores, bestIndices] = mink(RMSD_Scores, 5000);
RMSD_Angles = RMSD_Angles(:, bestIndices);
RMSD_RMSD = RMSD_RMSD(bestIndices);
RMSD_Coordinates = RMSD_Coordinates(:, bestIndices);

indices = clustering(RMSD_Coordinates);
RMSD_Angles = RMSD_Angles(:, indices);
RMSD_Scores = RMSD_Scores(indices);
RMSD_RMSD = RMSD_RMSD(indices);
RMSD_Coordinates = RMSD_Coordinates(:, indices);
seconds = seconds + toc;
disp("A total of "+size(RMSD_Angles,2)+" cluster centers.");
rtime = ceil(seconds/60);
disp("GA running time now "+rtime+" minutes.")

RMSD_candAngles = RMSD_Angles(:,RMSD_Scores<score_threshold);
RMSD_candScores = RMSD_Scores(RMSD_Scores<score_threshold);
RMSD_candRMSD = RMSD_RMSD(RMSD_Scores<score_threshold);
RMSD_candCoordinates = RMSD_Coordinates(:,RMSD_Scores<score_threshold);

filename = sprintf('After_RMSD_SA_clustering_%s_24res.mat', name);
save(filename, 'RMSD_Angles', 'RMSD_Coordinates', 'RMSD_RMSD', 'RMSD_Scores', 'RMSD_candAngles', 'RMSD_candScores', 'RMSD_candRMSD', 'RMSD_candCoordinates');

% Form initial GA population
population = 700;
[RMSD_Scores, bestIndices] = mink(RMSD_Scores, population/2);
RMSD_Angles = RMSD_Angles(:, bestIndices);
RMSD_RMSD = RMSD_RMSD(bestIndices);
RMSD_Coordinates = RMSD_Coordinates(:, bestIndices);

[Energy_Scores, bestIndices] = mink(Energy_Scores, population-length(RMSD_Scores));
Energy_Angles = Energy_Angles(:, bestIndices);
Energy_RMSD = Energy_RMSD(bestIndices);
Energy_Coordinates = Energy_Coordinates(:, bestIndices);

Scores = [Energy_Scores, RMSD_Scores];
Angles = [Energy_Angles, RMSD_Angles];
RMSD = [Energy_RMSD, RMSD_RMSD];
Coordinates = [Energy_Coordinates, RMSD_Coordinates];

candAngles = [Energy_candAngles, RMSD_candAngles];
candScores = [Energy_candScores, RMSD_candScores];
candRMSD = [Energy_candRMSD, RMSD_candRMSD];
candCoordinates = [Energy_candCoordinates, RMSD_candCoordinates];
...
while i <= M && stall < stall_threshold
    ...
    FILE = fopen("relax_"+name+"_Generation"+i+"_24res.flags", 'w');
    fprintf(FILE, "-in:file:l "+name+"_Generation"+i+"_list_24res.txt\n");
    fprintf(FILE, '-in:file:fullatom\n');
    fprintf(FILE, "-in:file:native /scratch/qz886/Clustercenters_24res/Pnear_Candidates/output_"+name+".pdb\n");
    fprintf(FILE, '-parser:protocol relax_Cartesian.xml\n');
    fprintf(FILE, '-parser:script_vars Nres=24\n');
    fprintf(FILE, "-out:file:scorefile score_"+name+"_Generation"+i+"_24res.sc\n");
    fclose(FILE);

    FILE = fopen(name+"_Generation"+i+"_list_24res.txt", 'w');
    for cand = 1 : N
        fprintf(FILE, name+"_Generation"+i+"_cand"+cand+"_24res.pdb\n");
    end
    fclose(FILE);

    FILE = fopen("run_Relax_"+name+"_Generation"+i+"_24res.sbatch", 'w');
    fprintf(FILE, "#!/bin/bash\n");
    fprintf(FILE, "#SBATCH --nodes=4\n");
    fprintf(FILE, "#SBATCH --tasks-per-node=12\n");
    fprintf(FILE, "#SBATCH --mem=20GB\n");
    ...
    fprintf(FILE, "srun --mpi=pmi2 /share/apps/rosetta/2020.46.61480/openmpi/intel/2020.46.61480/main/source/bin/rosetta_scripts.mpi.linuxiccrelease @relax_"+name+"_Generation"+i+"_24res.flags\n");
    fclose(FILE);

    [~, job_id] = system("sbatch run_Relax_"+name+"_Generation"+i+"_24res.sbatch");
    ...
    minutes_relax = minutes_relax + str2double(status(1));
    seconds_relax = seconds_relax + str2double(status(2));

    tic;
    FILE = fopen("score_"+name+"_Generation"+i+"_24res.sc", 'r');
    ...
    system("rm "+name+"_Generation"+i+"*_24res*.pdb");
    system("rm "+name+"_Generation"+i+"_list_24res.txt");
    system("rm relax_"+name+"_Generation"+i+"_24res.flags");
    system("rm run_Relax_"+name+"_Generation"+i+"_24res.sbatch");
    system("rm score_"+name+"_Generation"+i+"_24res.sc");
    system("rm slurm-"+job_id+".out");

    filename = sprintf('After_GA_%s_24res.mat', name);
    save(filename, 'Angles', 'Coordinates', 'RMSD', 'Scores', 'candAngles', 'candScores', 'candRMSD', 'candCoordinates');

    i = i + 1;
    rtime = ceil(seconds/60);
    disp("GA running time now "+rtime+" minutes.")
    retime = minutes_relax + ceil(seconds_relax/60);
    disp("Relaxation running time now "+retime+" minutes.")
end
end
...
for cand = 1 : N
    ...
    filename = sprintf(name+"_Generation%d_cand%d_24res.pdb", generation, cand);
