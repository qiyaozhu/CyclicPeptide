function Pnear_sampling_SA_24res(seq, name)
...
file = "/scratch/qz886/Clustercenters_24res/Pnear_Candidates/output_"+name+".pdb";
...
% Initial points for minimizing energies
SampleSize = 10000;
...
filename = sprintf('After_Energy_SA_%s_24res.mat', name);
save(filename, 'candPhi', 'candPsi', 'candOmega', 'candCoordinates_x', 'candCoordinates_y', 'candCoordinates_z', 'candScores');
fprintf('Number of candidates obtained from Energy SA is %d.\n', size(candAngles,2));

% Initial points for minimizing RMSD
SampleSize = 50000;
points = InitialPoints(seq, SampleSize);
candAngles = [];
candScores = [];
...
candPhi = rad2deg(candPhi);
candPsi = rad2deg(candPsi);
filename = sprintf('After_RMSD_SA_%s_24res.mat', name);
save(filename, 'candPhi', 'candPsi', 'candOmega', 'candCoordinates_x', 'candCoordinates_y', 'candCoordinates_z', 'candScores');
fprintf('Number of candidates obtained from RMSD SA is %d.\n', size(candAngles,2));
end
...
n = length(seq);
centers = deg2rad([-110, 60, -80, 110, -60, 80, -80, 80; ...
    90, 30, 60, -90, -30, -60, -60, 60]);
...
[x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24]=ndgrid(IP{1},IP{2},IP{3},IP{4},IP{5},IP{6},IP{7},IP{8},IP{9},IP{10},IP{11},IP{12},IP{13},IP{14},IP{15},IP{16},IP{17},IP{18},IP{19},IP{20},IP{21},IP{22},IP{23},IP{24});
bins = [x1(:),x2(:),x3(:),x4(:),x5(:),x6(:),x7(:),x8(:),x9(:),x10(:),x11(:),x12(:),x13(:),x14(:),x15(:),x16(:),x17(:),x18(:),x19(:),x20(:),x21(:),x22(:),x23(:),x24(:)];
...
if size(bins, 1) < SampleSize
    ...
    for pos = 1 : size(bins, 1)
        ...
        points(:,pos) = [centers(:,bin(1)); centers(:,bin(2)); centers(:,bin(3)); ...
	        ...
	        centers(:,bin(16)); centers(:,bin(17)); centers(:,bin(18)); ...
            centers(:,bin(19)); centers(:,bin(20)); centers(:,bin(21)); centers(:,bin(22)); ...
            centers(:,bin(23)); centers(:,bin(24))];
    end
else
