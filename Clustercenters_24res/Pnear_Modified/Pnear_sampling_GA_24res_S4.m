function Pnear_sampling_GA_24res_S4(seq, name)
...
% % Read from Cartesian relaxed pdb files
% FILE = fopen("After_SA_clustercenterS4-Design_0001_0001_24res.sc", 'r');
% data = textscan(FILE, '%s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %s', 'HeaderLines', 2);
% Scores = data{2}.';
% RMSD = data{24}.';
% Descriptions = data{26};
% fclose(FILE);
% 
...
% filename = sprintf('After_SA_clustering_%s_24res.mat', name);
...
population = 1000;
[Scores, bestIndices] = mink(Scores, 2*population);
Angles = Angles(:, bestIndices);
RMSD = RMSD(bestIndices);
Coordinates = Coordinates(:, bestIndices);
