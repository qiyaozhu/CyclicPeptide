...
fclose(FILE);
CenterNames = data{1};
Hpass = ones(1,length(CenterNames));
...
for ros = 1 : length(CenterNames)
    ...
    resName = backbone.resName;
    resNum = backbone.resNum;
    native_backbone = zeros(5*n,3);
    ...
    for i = 1 : n
        resNumCondition = resNum == i;
        aa = resName(resNum==i);
        aa = aa{1};
        if strcmp(aa, "PRO") || strcmp(aa, "DPR")
            H_coor(i,:) = [-1000,-1000,-1000];
        else
            atomNameCondition = strcmp(atomName, 'H');
            index = find(resNumCondition & atomNameCondition);
            H_coor(i,:) = coordinate(index,:);
        end
    end

    for res = 1 : n
        native_backbone(5*res-4,:) = N_coor(res,:);
        native_backbone(5*res-3,:) = H_coor(res,:);
        native_backbone(5*res-2,:) = Ca_coor(res,:);
        native_backbone(5*res-1,:) = C_coor(res,:);
        native_backbone(5*res,:) = O_coor(res,:);
    end

    [Hbonds, count] = E_hbond(native_backbone);
    br = Hbonds(:);
    br = sort(br);
    sep = br([2:end,1])-br;
    sep(sep<0) = sep(sep<0)+n;
    if sum(sep>2) > 0
        Hpass(ros) = 0;
    end
end

save("Hpass.mat", "Hpass");


function score = Hbond_score(Hbonds, n)
score = 0;
br = Hbonds(:);
br = sort(br);
sep = br([2:end,1])-br;
sep(sep<0) = sep(sep<0)+n;
score = score - sum(sep(sep>2)-2);

hd = abs(Hbonds(:,1)-Hbonds(:,2));
hd = min(hd, 24-hd);
score = score + sum(log(hd));

cross = 0;
sortedHbonds = sort(Hbonds, 2);
sortedHbonds = sortrows(sortedHbonds, 1);
for i = 1 : size(sortedHbonds,1)
    for j = i+1 : size(sortedHbonds,1)
        if sortedHbonds(j,1) > sortedHbonds(i,1) && sortedHbonds(j,1) < sortedHbonds(i,2) && sortedHbonds(j,2) > sortedHbonds(i,2)
            cross = cross + 1;
        end
    end
end
score = score + sum(log(cross));
end
...
% @ coordinates: size 5nx3 matrix, 5n atoms (N, H, CA, C, O)
function [Hbonds, count] = E_hbond(coordinates)
...
n = size(coordinates,1) / 5; % number of residues
for i = 1 : n % acceptor residue number
    for j = 1 : n % donor residue number
        if j ~= i

            % relevant atom coordinates
            B2 = coordinates(i*5-2,:); % Ca
            B = coordinates(i*5-1,:); % C
            A = coordinates(i*5,:); % O
            H = coordinates(j*5-3,:); % H
            D = coordinates(j*5-4,:); % N
