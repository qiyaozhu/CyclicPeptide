...
#             'Pnear_sampling_SA_20res("'+seq+'", "clustercenter'+name+'_0001_0001")\n'+
#             "delete(gcp('nocreate'))\n"+
#             "exit\n"+
#             "EOF")
...
with open('run_Relax_SA_'+name+'_20res.sbatch', 'w') as f:
    f.write('#!/bin/bash\n'+
            '#SBATCH --nodes=4\n'+
            ...
            'srun --mpi=pmi2 /share/apps/rosetta/2020.46.61480/openmpi/intel/2020.46.61480/main/source/bin/rosetta_scripts.mpi.linuxiccrelease @clustercenter'+name+'_0001_0001_SA_relax_rosetta_20res.flags')

# Perform FastRelax
job_id = subprocess.run(['sbatch', 'run_Relax_SA_'+name+'_20res.sbatch'], capture_output=True, text=True).stdout.strip()
job_id = job_id.split()[-1]
while True:
    job_status = subprocess.run(['squeue', '-j', job_id], capture_output=True, text=True).stdout.strip()
    job_status = job_status.split('\n')
...
Angles = []
with open("After_SA_clustercenter"+name+"_0001_0001_20res.silent", 'r') as f:
    f.readline()
    f.readline()
    lines = f.readlines()
...
np.savetxt('After_SA_clustercenter'+name+'_0001_0001_Angles_20res.txt', Angles, delimiter='\t', fmt='%f')

# Run Genetic Algorithm
with open('run_Pnear_sampling_GA_'+name+'_20res.sbatch', 'w') as f:
    f.write("#!/bin/bash\n"+
            ...
            "saveAsProfile(cluster, 'greene')\n"+
            "parallel.defaultClusterProfile('greene');\n"+
            'Pnear_sampling_GA_20res("'+seq+'", "clustercenter'+name+'_0001_0001")\n'+
            "delete(gcp('nocreate'))\n"+
            "exit\n"+
            "EOF")

job_id = subprocess.run(['sbatch', 'run_Pnear_sampling_GA_'+name+'_20res.sbatch'], capture_output=True, text=True).stdout.strip()
