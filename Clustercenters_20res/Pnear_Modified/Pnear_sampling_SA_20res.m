function Pnear_sampling_SA_20res(seq, name)
...
file = "/scratch/qz886/Clustercenters_20res/Pnear_Candidates/output_"+name+".pdb";
...
filename = sprintf('After_SA_%s_20res.mat', name);
...
cyc = f_cyc20(angles, R_omega, T_CR, T_NR, T_CaR, q);
[hbond, count] = E_hbond(coordinates);

if rep <= 13 && cyc <= 1 && count >= ceil(n/3)
...
while i <= M
    ...
    if rama_explore
        ...
        if rep_explore
            cyc_new = f_cyc20(angles_new, R_omega, T_CR, T_NR, T_CaR, q);
            cyc_explore = false;
...
[x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20]=ndgrid(IP{1},IP{2},IP{3},IP{4},IP{5},IP{6},IP{7},IP{8},IP{9},IP{10},IP{11},IP{12},IP{13},IP{14},IP{15},IP{16},IP{17},IP{18},IP{19},IP{20});
bins = [x1(:),x2(:),x3(:),x4(:),x5(:),x6(:),x7(:),x8(:),x9(:),x10(:),x11(:),x12(:),x13(:),x14(:),x15(:),x16(:),x17(:),x18(:),x19(:),x20(:)];
