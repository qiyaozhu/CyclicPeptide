...
with open('Pnear_Cartesian_20res.out', 'r') as f:
    plines = f.readlines()
...
for i in range(len(candidates)):
    path = 'After_GA_'+candidates[i]+'_20res.mat'
...
with open('Pnear_values_20res.txt', 'w') as f:
...
# with open('Pnear_candidates_20res.txt', 'w') as f:
#     f.write('Index\tName\t\t\t\tEnergy\t\tHbond\tRadius\tSequence\n')
#     for i in range(len(Pnear), len(candidates)):
#         if int(Hbonds[i])>=8 and Rg[i]<6.5:
#             f.write(str(i)+'\t'+candidates[i].split('_')[0]+f'\t\t{scores[i]:.3f}\t\t'+Hbonds[i]+f'\t{Rg[i]:.3f}\t'+seqs[i]+'\n')
...
plt.ylabel('Pnear GA')
plt.savefig('Pnear_Rg_20res.pdf')


# Pnear correlation plot
PnearM = PnearM[:len(Pnear)]
scores = scores[:len(Pnear)]
plt.rc('font', **font)
plt.figure(figsize=(10,10))
...
plt.savefig('Pnear_correlation_20res.pdf')
...
while l < len(plines):
    if len(plines[l].split()) > 0:
        if plines[l].split()[0] == "MPI_worker_node":
            ...
            Energy = []
            RMSD = []
            path = 'After_GA_'+candidates[count]+'_20res.mat'
            if os.path.exists(path):
                mat = scipy.io.loadmat(path)
                Energy = mat["candScores"][0]
                RMSD = mat["candRMSD"][0]
                ...
                Energy = []
                RMSD = []
                path = 'After_GA_'+candidates[count]+'_20res_reshape.mat'
                mat = scipy.io.loadmat(path)
                Energy = mat["Score"][0]
                RMSD = mat["RMSD"][0]
                p = sum(np.exp(-np.array(RMSD)**2 / lamda**2) * np.exp(-np.array(Energy) / kbT)) / sum(np.exp(-np.array(Energy) / kbT))
