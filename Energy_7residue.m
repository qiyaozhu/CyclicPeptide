...
[x1,x2,x3,x4,x5,x6,x7]=ndgrid(1:6,1:6,1:6,1:6,1:6,1:6,1:6);
bins = [x1(:),x2(:),x3(:),x4(:),x5(:),x6(:),x7(:)];
bin_class = unique(equivalent_class(bins), 'rows');
...
for bat = 0 : batches
    ...
    filename = sprintf('GoodAngles7res_%d.mat', bat+1);
    save(filename, 'GoodAngles', 'GoodRama', 'GoodRep', 'GoodCyc', 'GoodHbond', 'GoodCount', 'GoodEnergy');
end
end